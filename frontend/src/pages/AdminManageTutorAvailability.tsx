...
import { getTutorAvailability, updateTutorAvailability, deleteTutorAvailability } from "@/services/api";
import {
  Select,
  SelectContent,
  SelectItem,
...
import { Edit, Trash2, Plus, Save, X, AlertTriangle, RefreshCw } from "lucide-react";
...
const AdminManageTutorAvailability = () => {
  ...
  const [showDeleteConfirm, setShowDeleteConfirm] = useState<{ day: string; tutorName: string } | null>(null);
  ...
  const handleDeleteDay = async (day: Weekday) => {
    ...
    setDeletingDay(day);
    try {
      const res = await deleteTutorAvailability(selectedTutor._id, day);
      if (res.status === 200) {
  ...
  const confirmDeleteDay = (day: string) => {
    if (selectedTutor) {
      setShowDeleteConfirm({ day, tutorName: selectedTutor.name });
    }
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-red-600">Delete Availability</h2>
            </div>
            <p className="text-gray-700 mb-6">
              Are you sure you want to remove availability for <strong className="capitalize">{showDeleteConfirm.day}</strong> 
              from {showDeleteConfirm.tutorName}? This action cannot be undone.
            </p>
            <div className="flex justify-end space-x-3">
              ...
              <Button
                variant="destructive"
                onClick={() => handleDeleteDay(showDeleteConfirm.day as Weekday)}
                disabled={deletingDay === showDeleteConfirm.day}
              >
                {deletingDay === showDeleteConfirm.day ? (
                  <>

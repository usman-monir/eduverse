...
import { getAdminUsers, approveUser, inviteUser, restrictStudentAccess, enableStudentAccess, updateUserAccessibility, deleteUser } from '@/services/api';
...
const AdminUsers = () => {
  ...
  // New state for accessibility controls
  const [accessibilityDialogOpen, setAccessibilityDialogOpen] = useState<string | null>(null);
  const [accessibilitySettings, setAccessibilitySettings] = useState({
    canBookOneOnOne: true,
    canRequestSlots: true,
  });

  // New state for delete user dialog
  ...
  // New function to update student accessibility settings
  const handleUpdateAccessibility = async (userId: string) => {
    setRestrictingId(userId);
    try {
      await updateUserAccessibility(userId, accessibilitySettings);
      
      toast({
        title: 'Accessibility Updated',
        description: 'Student accessibility settings have been updated.',
      });
      
      setAccessibilityDialogOpen(null);
      await fetchUsers();
    } catch (error: any) {
      toast({
        title: 'Error',
        description: error.response?.data?.message || 'Failed to update accessibility settings.',
        variant: 'destructive',
      });
    } finally {
      setRestrictingId(null);
  ...
  // Function to open accessibility dialog with user's current settings
  const openAccessibilityDialog = (user: any) => {
    setAccessibilitySettings({
      canBookOneOnOne: user.canBookOneOnOne !== false,
      canRequestSlots: user.canRequestSlots !== false,
    });
    setAccessibilityDialogOpen(user._id);
  };
  ...
  const openDeleteDialog = (userId: string) => {
    setDeleteDialogOpen(userId);
    setDeleteConfirmation(''); // Reset confirmation when opening dialog
  };
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    {user.role !== 'admin' && (
                      <>
                        ...
                        {/* New Accessibility Controls Dialog */}
                        <Dialog open={accessibilityDialogOpen === user._id} onOpenChange={(open) => setAccessibilityDialogOpen(open ? user._id : null)}>
                          <DialogTrigger asChild>
                            <Button 
                              variant="outline" 
                              size="sm" 
                              className="text-blue-700 border-blue-400"
                              onClick={() => openAccessibilityDialog(user)}
                            >
                              Accessibility
                            </Button>
                          </DialogTrigger>
                          <DialogContent>
                            <DialogHeader>
                              <DialogTitle>Student Accessibility Settings</DialogTitle>
                              <DialogDescription>
                                Control what this student can do on the platform.
                              </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4">
                              <div className="flex items-center justify-between">
                                <div>
                                  <Label className="text-sm font-medium">Book One-on-One Sessions</Label>
                                  <p className="text-xs text-gray-500">Allow student to book available sessions</p>
                                </div>
                                <Switch
                                  checked={accessibilitySettings.canBookOneOnOne}
                                  onCheckedChange={(checked) => 
                                    setAccessibilitySettings(prev => ({ ...prev, canBookOneOnOne: checked }))
                                  }
                                />
                              </div>
                              ...
                              <div className="flex items-center justify-between">
                                <div>
                                  <Label className="text-sm font-medium">Request Custom Slots</Label>
                                  <p className="text-xs text-gray-500">Allow student to request custom time slots</p>
                                </div>
                                <Switch
                                  checked={accessibilitySettings.canRequestSlots}
                                  onCheckedChange={(checked) => 
                                    setAccessibilitySettings(prev => ({ ...prev, canRequestSlots: checked }))
                                  }
                                />
                              </div>
                              ...
                              <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-200">
                                <p className="text-sm text-yellow-700">
                                  <strong>Note:</strong> These settings take effect immediately and will restrict the student's ability to use these features.
                                </p>
                              </div>
                            </div>
                            
                            <div className="flex justify-end space-x-2 pt-4">
                              <Button variant="outline" onClick={() => setAccessibilityDialogOpen(null)}>
                                Cancel
                              </Button>
                              <Button
                                onClick={() => handleUpdateAccessibility(user._id)}
                                disabled={restrictingId === user._id}
                              >
                                {restrictingId === user._id ? 'Updating...' : 'Update Settings'}
                              </Button>
                            </div>
                          </DialogContent>
                        </Dialog>
              ...
              <Dialog open={deleteDialogOpen !== null} onOpenChange={(open) => setDeleteDialogOpen(open ? deleteDialogOpen : null)}>
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      {deleteDialogOpen && users.find(u => u._id === deleteDialogOpen) && (
                        <>
                          Are you sure you want to delete <strong>{users.find(u => u._id === deleteDialogOpen)?.name}</strong>? 
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-end space-x-2">
                      ...
                      <Button
                        ...
                        disabled={deletingUserId !== null || deleteConfirmation !== 'DELETE'}
                      >

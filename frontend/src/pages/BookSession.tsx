...
import { Calendar as CalendarIcon, Clock, User, Plus, Lock } from "lucide-react";
...
const BookSession = () => {
  ...
  const sessionsByDateAndTutor = (Array.isArray(sessions) ? sessions : []).reduce((acc, session) => {
  ...
  console.log("userBookings state:", userBookings);
  console.log("userBookings type:", typeof userBookings);
  console.log("userBookings isArray:", Array.isArray(userBookings));

  // Ensure userBookings is always an array to prevent errors
  const safeUserBookings = Array.isArray(userBookings) ? userBookings : [];
  
  const bookedDatesSet = new Set(
    safeUserBookings.map((booking: any) => {
      console.log("Processing booking:", booking);
      return new Date(booking.date).toDateString();
    })
  );
    ...
    const availableSessions = Object.entries(tutorSessions).flatMap(
      ([tutorName, sessions]) => {
        ...
        if (!Array.isArray(sessions)) {
          console.warn(`Sessions for tutor ${tutorName} is not an array:`, sessions);
          return [];
        }
  ...
  const groupSessionsByTutor = (sessions: any[]) => {
    // Ensure sessions is an array
    if (!Array.isArray(sessions)) {
      console.warn("groupSessionsByTutor: sessions is not an array:", sessions);
      return {};
    }
  ...
  const sessionsByDate = (Array.isArray(sessions) ? sessions : []).reduce((acc, session) => {
    if (session.status === "available") {
      const dateKey = new Date(session.date).toDateString();
      if (!acc[dateKey]) {
        acc[dateKey] = [];
  ...
  // Check if user has accessibility restrictions
  const isAccessRestricted = user?.canBookOneOnOne === false;
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {/* Accessibility Restriction Notice */}
        {isAccessRestricted && (
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-red-700 text-sm">
                    Your ability to book one-on-one sessions has been restricted by an administrator. 
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-green-700 text-sm">
                    ...
                    {(Array.isArray(sessions) ? sessions : []).filter((s) => s.status === "available").length}{" "}
                    total sessions
                  </p>
                </div>
                        ...
                        >
                          <h3 className="font-semibold text-lg mb-3 flex items-center">
                            ...
                            <Badge className="ml-2 bg-blue-100 text-blue-800">
                              {(tutorSessions as any[])?.length} session
                              {(tutorSessions as any[])?.length > 1 ? "s" : ""}
                            </Badge>
                          </h3>

                          <div className="space-y-3">
                            {(tutorSessions as any[]).map((session) => {

...
import { NotificationStats, SmartQuadBatch } from '@/types';
import { 
  Bell, 
  Mail, 
  Users, 
...
const AdminNotifications = () => {
  ...
  const [smartQuads, setSmartQuads] = useState<SmartQuadBatch[]>([]);
  const [loading, setLoading] = useState(true);
  const [sendingNotifications, setSendingNotifications] = useState(false);
  const [selectedSmartQuad, setSelectedSmartQuad] = useState<SmartQuadBatch | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      const [statsRes, smartQuadsRes] = await Promise.all([
        getNotificationStats(),
        getSmartQuadBatches({ status: 'active', limit: 100 })
      ]);
                        ...
                        >
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {smartQuad.preferredLanguage} • Score: {smartQuad.desiredScore}
                            </p>
                          </div>
